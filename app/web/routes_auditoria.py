...
from app.core.rbac import PAPEIS_IMPEDIMENTO_LEITURA, cpl_ids_visiveis, verificar_papel
...
def selecionar_cpl(
    ...
    return templates.TemplateResponse(
        request, "restrito/auditoria/cpls.html", {"cpls": cpls, "usuario": usuario, "pagina_ativa": "auditoria"}
    )
...
def listar_registros(
    ...
    if acao_filtro is not None:
        query = query.filter(RegistroAuditoria.acao == acao_filtro)
    if entidade_tipo:
        query = query.filter(RegistroAuditoria.entidade_tipo == entidade_tipo)
    registros = query.order_by(RegistroAuditoria.created_at.desc()).limit(LIMITE_PADRAO).all()
