...
| RF-045 | Disponibilizar painéis de governança, maturidade, projetos, finanças e impacto territorial. | M | ⚠️ Parcial — painel de governança (`/painel`) + painel consolidado de governança/planejamento/cadastro/**projetos e finanças** por CPL (`/painel/indicadores/cpls/{id}`). O card "Projetos" novo agrega todo o portfólio da CPL (não um projeto só, que já era coberto pelos relatórios do RF-041): contagem por estágio/prioridade, financeiro (previsto/desembolsado/saldo somados de todas as origens de recurso e desembolsos dos projetos da CPL) e execução (etapas/marcos/entregas/metas/riscos agregados) — `resumo_projetos_cpl()` em `app/services/projeto.py`. Projetos também mantém a tela de portfólio individual (`/painel/projetos/cpls/{id}`) e o detalhe por projeto (RF-034 a RF-041). Só **maturidade** ainda não tem painel próprio — não foi priorizado ainda |
...
| RNF-010 – Interoperabilidade | APIs REST/JSON, webhooks quando cabíveis e exportação em formatos abertos. | ⚠️ REST/JSON ok; exportação de entidades em XLSX/CSV (RF-053) implementada; webhooks e exportação de outras listagens pendentes |
...
| Fase 3 – Execução e fomento (2–3 meses) | Execução física/financeira, prestação de contas, riscos, bens, relatórios e portal de transparência. | ⚠️ **Maior parte feita já na Fase 2** — execução física/financeira, prestação de contas, riscos (com evidência) e relatórios (RF-041, ver Fase 2) foram construídos junto com o resto do módulo de Projetos, em vez de esperar uma fase separada; "bens" tem só `bem_adquirido` (texto livre em `DesembolsoProjeto`), sem controle patrimonial próprio. Falta: portal de transparência (RF-055, expansão do portal público hoje só institucional/estático) |
