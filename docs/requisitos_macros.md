...
| RF-045 | Disponibilizar painéis de governança, maturidade, projetos, finanças e impacto territorial. | M | ⚠️ Parcial — painel de governança (`/painel`) + painel consolidado de governança/planejamento/cadastro por CPL (`/painel/indicadores/cpls/{id}`). Projetos tem uma tela de portfólio (`/painel/projetos/cpls/{id}`, lista + filtros visuais) e detalhe com plano de trabalho, financeiro, execução e prestação de contas completos (RF-034 a RF-041, incluindo relatórios de execução/financeiro/dossiê de evidências por projeto), mas não um "painel" agregando todos os projetos de uma CPL como os outros módulos. Maturidade, finanças e impacto territorial (agregados entre projetos) não têm painel próprio — maturidade porque não foi priorizado ainda, finanças/impacto territorial porque exigiriam consolidar os relatórios por projeto do RF-041 numa visão de CPL, ainda não construída |
